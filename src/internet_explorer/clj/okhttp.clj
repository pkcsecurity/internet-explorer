(ns internet-explorer.clj.okhttp
  (:import [okhttp3
...
(defn response-type-handler [response-type response]
  (case response-type
    :json (parse-string (.. response body string))
    :clj (parse-string (.. response body string) true)
...
(defn async-req [method url & {:keys [body
                               ...
                               :or {media-type "application/json"
                                    ...
                                    body {}
                                    response-type :clj
                                    on-success #(println %2 %3)
                                    on-error #(println %2 %3)
                                    on-exception #(println %2 %3)}
                               :as opts}]
